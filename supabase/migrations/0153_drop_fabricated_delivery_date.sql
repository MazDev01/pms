-- Benjamin PMS - ตัด "วันส่งมอบ" ออกจากฐานข้อมูล (บอสสั่ง 20 ส.ค. 69)
--
-- ต้นเหตุ: ไม่มีที่ให้ใครกรอกวันส่งมอบจริงเลยทั้งฝั่งตัวแทนและสำนักงานใหญ่
--   ค่าที่ระบบเคยโชว์คือ (วันปิดการขาย + 90 วัน) ที่คิดขึ้นเองตรงนี้ในฐานข้อมูล
--   = ตัวเลขที่ไม่มีอยู่จริงในงาน ผิดกติกาหลักของโปรเจกต์ที่ห้ามแสดงข้อมูลไม่มีที่มา
--
-- ฝั่งแอปเลิกอ่านค่านี้แล้ว ใบนี้ตัดที่ต้นทางด้วย เพื่อไม่ให้ใครหยิบกลับมาใช้อีก
--   - เลิกคิด delivered_at และ delivery_years
--   - คงพารามิเตอร์ p_delivery_year ไว้เฉย ๆ (ผู้เรียกเก่าที่ส่ง null มาจะได้ไม่พัง) แต่ไม่มีผลแล้ว
--
-- ฐานของใบนี้คือ 0104 (ตัวล่าสุด) คัดลอกมาทั้งดุ้นแล้วตัดเฉพาะส่วนวันส่งมอบ
-- เพื่อไม่ให้การแก้ครั้งก่อน (join ข้ามสาขา) หายไปโดยไม่ตั้งใจ
