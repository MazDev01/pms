-- Benjamin PMS — M9 Phase 4: เพิ่ม latest (วันที่ใบล่าสุดรายสาขา) ใน hq_quotations_summary.byDealer
-- ป้อน pipeline perf.latest ("ใบเสนอราคาล่าสุด" รายสาขา) — เดิมคิด client จาก quotes array
...
as $$
  with f as (
    select
      ...
      total_value,
      product_line,
      case when date ~ '^\d{4}-\d{2}-\d{2}' then substring(date, 1, 10)::date end as qdate,
