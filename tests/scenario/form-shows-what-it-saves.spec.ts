...
//   ฟอร์มลีดแสดงว่าเลือก "โกดังสำเร็จรูป" ไว้แล้ว แต่ค่าจริงในระบบเป็นว่าง
...
test("[func] ฟอร์มลีด: แม่แบบที่หน้าจอโชว์ ต้องเป็นค่าที่ลงฐานข้อมูลจริง", async ({ page }) => {
  ...
  await expect(page.getByLabel("แม่แบบที่สนใจ").first()).toBeVisible();
  ...
  const sel = page.getByLabel("แม่แบบที่สนใจ").first();
...
test("[func] ฟอร์มลีด: มูลค่าที่กรอกแล้วอ่านไม่ออก ต้องฟ้อง ไม่ใช่บันทึกเป็น 0 เงียบ ๆ", async ({ page }) => {
  const sb = await db(RYG);
  ...
  // ต้องเห็นข้อความบอกเหตุผล และต้องไม่มีลีดลงฐานข้อมูล
