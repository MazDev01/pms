...
test("[dealer] ลืมรหัสผ่าน — อีเมลออกจริงและลิงก์พาไปหน้าตั้งรหัสใหม่ (เฉพาะฐานในเครื่อง)", async ({ page }) => {
  ...
  const ส่งแล้ว = await page.getByText(/ส่งลิงก์ตั้งรหัสผ่านใหม่ไปที่/).isVisible({ timeout: 1 }).catch(() => false)
    || await page.getByText(/ส่งลิงก์ตั้งรหัสผ่านใหม่ไปที่/).waitFor({ state: "visible", timeout: 20_000 }).then(() => true, () => false);
  if (!ส่งแล้ว) {
