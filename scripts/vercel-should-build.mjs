...
// 0) สาขาที่ไม่ใช่สาขาที่ใช้ขึ้นเว็บจริง = ไม่ต้อง build (บอสเจอโควตาเต็ม 2 ก.ย. 69)
//
//    เราอัปสาขาหลัก (main) ตามสาขางาน (monorepo) เพื่อให้งานตามเวลาของ GitHub ทำงานได้
//    (GitHub รันงานตามตารางจาก "สาขาหลัก" เท่านั้น) แต่ Vercel เห็น push นั้นแล้วสร้าง
//    Preview ให้ทุกโปรเจกต์ทุกครั้ง — วันเดียวกินโควตาแผนฟรี (100 deploy/วัน) จนเต็ม
//    แล้วอัปเว็บ demo ไม่ได้เลย ("Resource is limited - try again in 1 day")
//
//    Preview ของสาขาหลักไม่มีใครดูอยู่แล้ว (เว็บจริงมาจากสาขา monorepo) — ข้ามทิ้งได้เต็มปาก
const สาขาที่ขึ้นเว็บจริง = process.env.PMS_DEPLOY_BRANCH ?? "monorepo";
if (branch !== "(ไม่ทราบ)" && branch !== สาขาที่ขึ้นเว็บจริง) {
  ข้าม(`คอมมิตนี้อยู่สาขา ${branch} ไม่ใช่ ${สาขาที่ขึ้นเว็บจริง} ที่ใช้ขึ้นเว็บจริง`);
}
