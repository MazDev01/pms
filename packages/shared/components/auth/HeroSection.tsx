...
export default function HeroSection({ variant = "dealer" }: { variant?: "dealer" | "hq" }) {
  ...
  return (
    <div className="relative flex min-h-[600px] flex-col overflow-hidden">
      ...
      {/* wash gradient — ม่านขาวบางให้ตัวอักษรอ่านออกบนพื้นรูป
          ⚠️ ค่าปัจจุบันคือ "จุดสมดุล" หลังเอาขอบเรืองขาวรอบตัวอักษรออก (2 ก.ย. 69):
             เคยลดม่านลงมาก (0.36/0.26/0.18) แล้วชดเชยด้วย halo รอบตัวอักษร — บอสเห็นเป็นขอบขาวไม่สวย
             จึงเพิ่มม่านกลับมาบางส่วน แต่ยังโปร่งกว่าชุดก่อนหน้า (0.58/0.44/0.34) รูปจึงยังชัดกว่าเดิม
             ถ้าจะลดม่านอีก ต้องเช็กว่าย่อหน้าคำอธิบายที่ทับหลังคาสีน้ำเงินยังอ่านออก (คอนทราสต์ต่ำสุด) */}
      <div className="absolute inset-0 bg-[linear-gradient(to_bottom,rgba(238,244,252,0.50)_0%,rgba(238,244,252,0.40)_50%,rgba(238,244,252,0.30)_100%)]" />
      {/* ม่านเสริมเฉพาะครึ่งล่าง — โปร่งครึ่งบน (คงรูป/ท้องฟ้า) เข้มขึ้นครึ่งล่างที่พื้นเข้ม ให้คำอธิบายล่างอ่านคม */}
      <div className="absolute inset-0 bg-[linear-gradient(to_bottom,transparent_0%,transparent_48%,rgba(238,244,252,0.30)_74%,rgba(238,244,252,0.48)_100%)]" />

      {/* เนื้อหา — flex-1 + justify-between: กลุ่มแบรนด์ชิดบน · ฟีเจอร์ดันลงล่าง (เต็มแผง ไม่เหลือรูปว่าง)
          ระยะห่างย่อ ~90% แต่ขนาดฟอนต์คงเดิม
          ⚠️ ขอบเรืองขาวรอบตัวอักษร (text-shadow) เอาออกแล้วตามที่บอสสั่ง 2 ก.ย. 69 — มองเห็นเป็น
             "ขอบขาว" รอบตัวหนังสือชัดเจนบนจอใหญ่ ตัวอักษรจึงต้องอ่านออกด้วยม่านขาวอย่างเดียว
